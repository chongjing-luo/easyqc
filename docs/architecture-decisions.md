...
| 007 | 冻结包定位 | 已采纳（已演进） | PyInstaller 不取代托管运行时；它作为原生便利安装包的应用冻结层，由每个目标系统原生构建 |
...
| 017 | 原生便利安装包 | 已采纳（发布证据积累中） | 同一源码生成 Ubuntu `.deb`、Windows per-user Setup 和 macOS arm64 DMG；每包带 manifest/SHA-256，未签名状态必须显式 |
...
- 源码使用项目内 `.venv`，从仓库根运行 `python easyqc.py`；Docker 暂不
  使用。PyInstaller 只负责原生便利包的应用冻结层，托管运行时仍是独立的
  长期运行时方向，两者都不能在对应发布矩阵完成前写成已签名公开发行事实。
...
- 原生包构建合同和 Linux `.deb` 已有本机证据；Windows/macOS 安装包只能在
  对应原生 runner 结果完成后报告，签名/公证仍是独立发布门槛。
