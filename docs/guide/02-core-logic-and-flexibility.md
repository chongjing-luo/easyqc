...
质控任务指具体的检查内容；质控任务模块（QC module，简称模块）是组织这些内容的软件配置单元。一个模块可以只检查影像伪影，也可以用多个评分项分别记录伪影、脑提取和表面重建质量。多个检查共用条目队列、命令配置和一份模块级备注；当前没有独立子任务对象或每个评分项各自的备注字段。

显示标签可以面向人类使用中文或长文本；内部 `module_name` 则使用短、稳定、可移植的 ASCII 标识。可配置的是检查范围、呈现方式与已有评分结构；新的影像转换或新的评分控件仍可能需要外部准备或开发。
...
跨项目设置中可保存常量模板和模块模板。源码运行且未设置 `EASYQC_DATA_ROOT` 时，它们位于当前源码目录：
...
冻结应用默认使用按版本隔离的用户数据目录；显式设置 `EASYQC_DATA_ROOT` 时使用该目录。具体规则由 `core/application_paths.py` 决定，不能将源码目录规则套用于所有安装方式。
...
- 宽表每个当前名单条目一行，保留未保存评分的条目；长表每个已保存的条目、模块和评分者组合一行，并附加当前总名单字段。
- 删除名单行后，对应评分仍保留，但不再出现在当前长表或宽表中；内部原始评分展平表与面向用户的长表不是同一个数据范围。
...
模块快照不包含当时的完整总名单、项目常量表、应用级 Shell 选择、查看器版本或窗口交互状态。重新打开已有评分时，保存的筛选规则仍作用于当前总名单，命令仍使用当前条目字段和项目常量。固定某轮检查范围或复现旧环境，需要另行保存名单、配置、外部脚本及环境信息。
...
- 查看器模板中的 `${name}`、`{name}`、`$name` 只替换已知变量，未知名称原样保留；Shell 局部变量可在脚本中赋值后展开。拼写错误也不会被模板层拒绝，应检查命令日志。
