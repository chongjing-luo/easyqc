...
双击常量行或点击编辑后，名称和值均可修改。保存只修改该常量，不列出受影响模块、不自动修改模块命令中的引用；需要时由用户自行调整命令。新增同名常量不会覆盖原值。
...
质控任务模块把检查范围、呈现方式、评分结构与评分者组合在一起。一个模块可以组织一个或多个相关检查任务；这些检查共享队列与启动配置，由多个评分项和标签区分检查内容。
...
| tags | 一个或多个布尔标记 |
| notes | 当前条目在该模块中的单一自由文本备注，不是每个评分项独立备注 |
...
模块模板保存在应用状态根的 `modules/`，每个模板一个 JSON。源码运行默认使用当前仓库；冻结应用默认使用版本隔离的用户数据目录，详见[核心逻辑](02-core-logic-and-flexibility.md)。项目页选择“从模板添加”时：
...
项目模块也可导出为 JSON，再导入其他项目。模块文件不打包总名单、项目常量、查看器或外部脚本；复用时应检查字段名、路径、评分定义以及“跨项目设置”中的 Shell 选择。复制成功并不表示目标环境已能打开数据。

新建模块时，命令、质控员、评分项、标签及进程控制选项可按任意顺序填写。保存会一次提交完整草稿，失败保留输入，不先创建空白模块。
...
推荐 `${name}`，因为边界最清楚。对于以上三种形式，名称存在于项目常量或当前名单行时替换，不存在时完整保留，不报错。Shell 模式下，保留的变量由 Shell 继续解释；直接模式下作为字面参数传入。Shell 局部变量应避免与常量/名单列同名，以免被提前替换。拼错的未知名称也会保留，而 Shell 中未定义变量通常展开为空，可结合命令输出排查。
...
它在直接执行和 Shell 模式下都可使用。`start_commands()` 按顺序调用进程启动，每个命令经过短暂启动检查后继续，因此长时间运行的查看器可以共存；这不保证窗口同时就绪，也不是等待每个查看器关闭后才启动下一个。单纯换行不会在直接执行模式下自动变成多个独立命令。

若模块启用进程控制，切换记录时 EasyQC 可关闭上一个由它托管的进程组，再启动下一条命令。这适合“每个 QC 条目只保留一个查看器实例”的流程。未被 EasyQC 托管的外部进程不在这一生命周期内。

## 11. 在质控窗口查看命令输出

质控窗口底部提供可折叠的“命令输出”面板，用于观察查看器命令、标准输出、标准错误和退出状态。面板默认收起；EasyQC 会记住上次的展开/收起状态，并在展开时恢复最近使用过的有效高度。启动查看器不会强制展开或收起面板，收起期间也会继续采集输出。

面板约每 200 ms 拉取一次新记录。每行带有命令 ID（例如 `C001`）以及下列类型前缀，便于把并行或连续执行的消息对应到同一条命令：

| 前缀 | 含义 |
|---|---|
| `START` | 一次命令执行开始，并记录模块、质控员、`easyqcid` 和命令序号 |
| `CMD` | 展开后实际提交执行的命令 |
| `OUT` | 外部程序的标准输出 |
| `ERR` | 外部程序的标准错误 |
| `EXIT` | 进程退出及退出码 |
| `WARN` | EasyQC 在采集或持久化过程中发现的诊断警告 |

面板操作的边界如下：

- “复制全部”复制当前面板中仍可见的全部文本；
- “清空显示”只清除当前界面内容，不删除磁盘日志，也不停止仍在运行的命令；
- “打开日志”打开本次 EasyQC 应用会话的完整查看器命令日志；持久化不可用时该按钮不可用，面板会显示退化状态；
- 为避免长时间运行使 GUI 无限制增长，面板只保留最新 5,000 行；完整磁盘会话日志不受这一界面上限截断；
- 查看器命令日志保留 14 天，过期的 EasyQC 自有会话日志和采集临时目录会在后续会话启动时清理。

EasyQC 能定期读取外部程序已经写出的内容，但不能强制外部程序刷新自己的 stdout/stderr 缓冲。因此某些程序会延迟显示输出，或直到产生足够内容、显式 flush、退出后才显示最后一批消息。这种延迟不等于命令没有执行。
...
| EasyQC 中找不到占位符名称 | 原样保留，后续按当前执行模式处理 |
| 可执行程序不存在 | 保留操作系统启动错误 |
