...
4. 使用原生冻结包时，从这个已初始化的同一终端启动 EasyQC 再测试；
5. 用 `command -v freeview` 等命令取得真实可执行文件，并在模块中测试绝对路径；
6. 检查 `FREESURFER_HOME`、`SUBJECTS_DIR`、`DISPLAY` 和 `XAUTHORITY` 是否在
   EasyQC 启动前已经存在；EasyQC 不自动读取交互式 Shell 配置；
7. `shell=False` 时不要使用管道、重定向或 `&&`；
8. 只有确实需要 Shell 语法或显式 `source` 初始化脚本时才启用 `shell=True`；
9. 检查引号和含空格参数；
10. 查看界面错误和当天日志中的展开、退出码与受限 stderr 摘要。

冻结版会在外部查看器子进程中移除 EasyQC 私有 Qt 插件路径，并恢复
PyInstaller 保存的原始 Linux 动态库路径。若错误仍出现，应报告完整退出码和
stderr 摘要，而不是在模块中长期手工拼接 EasyQC 的 `_internal` 路径。
