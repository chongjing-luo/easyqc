...
`LEFT`、`RIGHT`、`MID` 等文本函数的结果始终按文本处理。例如：

```text
RIGHT("scan02", 2)          → 文本 "02"
VALUE(RIGHT("scan02", 2))   → 数字 2
```

新增文本列在保存、刷新和重新打开项目后仍保留前导零。文本 `"12"` 也不会仅因
看起来像数字就变为数值；要计算时使用 `VALUE([列名])`。已有真正的数值列继续
按数值参与运算、排序和筛选。空字符串与缺失值也分别保存。

为使 CSV 重载时保留类型，EasyQC 会在表格旁保存类型记录，例如：

```text
Table/easyqc_all.csv
Table/easyqc_all.csv.types.json
```

CSV 仍保存最终值，JSON 只记录必要的类型/空值信息，不保存公式，也不复制单元格
内容。复制或备份项目表格时请同时保留这两个文件。类型记录最多保留对应当前和
候选 CSV 的两份元信息，用于安全提交；这不是评分历史或公式历史。

无类型记录的旧 CSV 仍按原有导入规则读取；本次修复不能从已经变为 `2` 的历史值
反推出原来是 `02` 还是 `002`。已有类型记录与 CSV 不匹配或损坏时，软件会明确
报错，不会静默重猜类型。若确需在外部修改名单，请先保留原文件，将修改后的 CSV
以新文件名通过导入预览检查，再明确替换/合并；新文件没有类型记录时须重新检查
文本列，避免外部编辑器或导入推断已去掉前导零。

导入携带匹配类型记录的 CSV 会保留这些类型；普通导出的 CSV 和第三方软件
（例如 Excel）不一定识别 EasyQC 的类型 JSON，外部打开时仍需将编号列设为文本。
旧版本 EasyQC 不识别这份类型记录，请不要用旧版本继续改写同一份名单。
